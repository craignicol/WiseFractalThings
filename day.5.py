...
def intcode_run(ram, inputs=[]):
    ...
    while ram[current] % 100 != OPCODE_END:
        ...
        if next_opcode in [OPCODE_ADD, OPCODE_MULT, OPCODE_LESSTHAN, OPCODE_EQUALS]:
            x, y, z = ram[current+1:current+4]
        elif next_opcode in [OPCODE_JUMPIFTRUE, OPCODE_JUMPIFFALSE]:
            x, y = ram[current+1:current+3]
            z = 0
        elif next_opcode in [OPCODE_READ, OPCODE_WRITE]:
            x = [ram[current+1]]
            y, z = 0, 0
        else:
            x, y, z = 0, 0, 0
        ...
        xval = x if xm == 1 else ram[x]
        yval = y if ym == 1 else ram[y]
        ...
        if next_opcode == OPCODE_ADD:
            ...
            current = yval if xval != 0 else current
        ...
        if next_opcode in [OPCODE_ADD, OPCODE_MULT, OPCODE_LESSTHAN, OPCODE_EQUALS]:
            current += 4
        elif next_opcode in [OPCODE_JUMPIFFALSE, OPCODE_JUMPIFTRUE]:
            current += 3
        elif next_opcode in [OPCODE_READ, OPCODE_WRITE]:
            current += 2
        elif next_opcode == OPCODE_END:
            current += 1        

    return ram, output

def execute():
    return intcode_run(input2, [1])[1]
...
def test_cases():
    verify([], intcode_run([]))
    verify([1], intcode_run([3,0,4,0,99], [1])[1])
    verify([-5], intcode_run([3,0,4,0,99], [-5])[1])
    verify([2,0,0,0,99], intcode_run([1,0,0,0,99])[0])
    verify([2,3,0,6,99], intcode_run([2,3,0,3,99])[0])
    verify([2,4,4,5,99,9801], intcode_run([2,4,4,5,99,0])[0])
    verify([30,1,1,4,2,5,6,0,99], intcode_run([1,1,1,4,99,5,6,0,99])[0])

    verify([0], intcode_run([3,9,8,9,10,9,4,9,99,-1,8], [7])[1])
    verify([1], intcode_run([3,9,8,9,10,9,4,9,99,-1,8], [8])[1])
    verify([1], intcode_run([3,9,7,9,10,9,4,9,99,-1,8], [7])[1])
    verify([0], intcode_run([3,9,7,9,10,9,4,9,99,-1,8], [8])[1])
    verify([0], intcode_run([3,3,1108,-1,8,3,4,3,99], [7])[1])
    verify([1], intcode_run([3,3,1108,-1,8,3,4,3,99], [8])[1])
    verify([1], intcode_run([3,3,1107,-1,8,3,4,3,99], [7])[1])
    verify([0], intcode_run([3,3,1107,-1,8,3,4,3,99], [8])[1])

    verify([0], intcode_run([3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9], [0])[1])
    verify([1], intcode_run([3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9], [-5])[1])
    verify([0], intcode_run([3,3,1105,-1,9,1101,0,0,12,4,12,99,1], [0])[1])
    verify([1], intcode_run([3,3,1105,-1,9,1101,0,0,12,4,12,99,1], [100])[1])
    verify([1000], intcode_run([3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99], [8])[1])
    verify([999], intcode_run([3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99], [-8])[1])
    verify([1001], intcode_run([3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99], [88])[1])
