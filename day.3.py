...
def trace(path):
    ...
    for line in path:
        ...
        if (direction == "R"):
            tracing.update({(x1, y) : distance+x1-x for x1 in range(x+1, x+magnitude+1)})
            x += magnitude
        elif (direction == "L"):
            tracing.update({(x1, y) : distance+x-x1 for x1 in range(x-magnitude, x)})
            x -= magnitude
        elif (direction == "D"):
            tracing.update({(x, y1) : distance+y1-y for y1 in range(y+1, y+magnitude+1)})
            y += magnitude
        elif (direction == "U"):
            tracing.update({(x, y1) : distance+y-y1 for y1 in range(y-magnitude, y)})
            y -= magnitude
        distance += magnitude
    return tracing
...
def test_cases():
    verify(6, closest_cross("R8,U5,L5,D3", "U7,R6,D4,L4"))
    verify(159, closest_cross("R75,D30,R83,U83,L12,D49,R71,U7,L72","U62,R66,U55,R34,D71,R55,D58,R83"))
    verify(135, closest_cross("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51","U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"))
    verify(30, shortest_cross("R8,U5,L5,D3", "U7,R6,D4,L4"))
    verify(610, shortest_cross("R75,D30,R83,U83,L12,D49,R71,U7,L72","U62,R66,U55,R34,D71,R55,D58,R83"))
    verify(410, shortest_cross("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51","U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"))
